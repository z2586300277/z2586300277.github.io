...
description: "Three.js 接第三方库或扩展能力。主流程在 `animate`、`createShapeWithCoord`。"
head:
  - - meta
    - name: keywords
      content: "three.js,cesium,webgl,地理边界,扩展功能"
outline: deep
---
...
Three.js 接第三方库或扩展能力。主流程在 `animate`、`createShapeWithCoord`。
...
function coordToVector3(coord, slace = 10000) {
  ...
  const y = Math.log(Math.tan((90 + lat) * Math.PI / 360)) / (Math.PI / 180) * 2003
```
